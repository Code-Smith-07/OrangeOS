...
| Control Center presentation | Native top-right glass transient panel with three palettes, grouped radio/battery tiles and Display/Sound sliders; no title bar, dragging, dock or overview entry; outside click/Escape/menu toggle dismiss with slider cancellation | Guest presentation and bridge regression suites cover current behavior. Radio operations, media, Focus and sharing remain separate backend work, not decorative toggles |
...
Finish physical-slider acceptance for the qualified built-in display compatibility
backend; do not treat the private API as a stable cross-version driver contract.
