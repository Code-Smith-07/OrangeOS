...
| Full desktop idle memory | ≤ 3 GiB | **85.77 MB** | hard, configurable | `budget.reportMemory` |
| Boot to scheduler | < 2 s | 2.245 s † | timing | `budget.reportBoot` |
| Context switch | < 500 ns | **22 ns** | timing † | `budget.benchContextSwitch` |
| Syscall round-trip | < 200 ns | **164 ns** † | timing | `userland/bin/bench` |
| Idle CPU (desktop shown) | < 1 % (goal) | **0.33 %** | advisory | `budget.benchIdleCpu` |
